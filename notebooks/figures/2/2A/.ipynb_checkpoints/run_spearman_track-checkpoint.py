    ...
    for i in range(len(r_frame)): # for each region in reference
        ...
        if cellID[i] != 0:
            ...
            if len(cdist2[np.where(cdist2==min(cdist2))]) > 1:
                if uy[edist2==min(edist2[np.where(cdist2==min(cdist2))])] != r_frame["label"][i]:
                    cellID[i] = 0
                    scores[i] = 2

            if (len(cdist2[np.where(cdist2==min(cdist2))]) == 1) and (uy[np.where(cdist2==min(cdist2))] != r_frame["label"][i]):
                cellID[i] = 0
                scores[i] = 2
...
for c in tqdm(cores): #iterate cores
    try:
        slide_1_table = pd.read_csv(f'/home/groups/ChangLab/heussner/tma-integration/data/nuc_cell_tables/{c}_tCyCIF_tumor_both.csv') # path to tables
        ...
        table.to_csv(f'/home/groups/ChangLab/heussner/coexist/figures/2/2A/Cell_tracking_correlation/{c}.csv',index=False)
