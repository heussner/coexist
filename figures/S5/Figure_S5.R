#### -------
#### Use the rviz.yml environment 
#### -------

library(tidyr)
library(dplyr)
library(ggalluvial)
library(scales) 

fh <- 'data/E06_propagated_combined_alluvial.csv'
df <- read.csv(fh)

df$label_origin <- factor(df$label_origin, levels = c('Original','Propagated'))

df$cell_type <- factor(
    df$cell_type, 
    levels = c(
        'Epithelial', 
        'Basal Epithelial', 
        'Proliferative Epithelial', 
        'Proliferative Basal Epithelial',
        'Myoepithelial', 
        'Stroma',
        'Endothelial',
        'aSMA+ Stroma', 
        'B-cells', 
        'T-cells', 
        'CD8+ T-cells', 
        'CD4+ T-cells',
        'Regulatory T-cells',
        'Macrophages', 
        'CD163+ Macrophages'
    )
)

cell_type_colors <- c(
    '#d60000', 
    '#8c3bff', 
    '#018700',
    '#00acc6', 
    '#97ff00', 
    '#ff7ed1', 
    '#6b004f',
    '#ffa52f', 
    '#00009c',
    '#857067', 
    '#004942', 
    '#4f2a00',
    '#00fdcf', 
    '#bcb6ff',
    '#95b379'   
)

names(cell_type_colors) <- levels(df$cell_type)
colScale <- scale_fill_manual(name = "Cell type", values = cell_type_colors)

# plot alluvial 
ggplot(df,
    aes(x = label_origin, 
        stratum = cell_type, 
        alluvium = subject,
        y = frequency,
        fill = cell_type, 
        label = cell_type)) +
    scale_x_discrete(expand = c(.1, .1)) +
    geom_flow(alpha = .8) +
    geom_stratum(alpha = .8) +
    facet_wrap(~panel) +
    theme_bw() + 
    theme(plot.title = element_text(hjust = 0.5)) +
    xlab('') + 
    ylab('Cell type frequency') +
    scale_y_continuous(labels = function(x) format(x, scientific = FALSE)) +
    theme(axis.text.x = element_text(size = 12, color = 'black')) + 
    colScale


ggsave('Figure_S5.png', dpi = 500, width = 10, height = 7)
