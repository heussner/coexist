...
### Rank marker plots 
rank_fh <- 'E06_aSMA_Macrophage_ID_switch_ranks.csv'
...
g2 <- ggplot(rank_df, aes(x = rank, y = scores, label = names)) +
    geom_text(aes(color = marker_origin),hjust=0, vjust=0, angle = 90, fontface = 'bold', size = 3) +
...
# library(grid)
# gt = ggplot_gtable(ggplot_build(g2))
# gt$widths[2] = 4*gt$widths[2]


ggsave('Figure_4D.png', dpi = 500, arrangeGrob(g1,g2), width = 9, height = 6)
